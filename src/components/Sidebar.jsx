  ...
  const content = (
    <aside className={`sidebar${collapsed ? ' collapsed' : ''}`}>
      ...
      <div className="sb-header">
        ...
        <button className="sb-toggle" onClick={toggle} title={collapsed ? 'Expandir' : 'Colapsar'}>
          <ChevronLeft size={16} />
        </button>
      </div>
      ...
      {TIENDITA.map(({ to, end, icon: Icon, label }) => (
        <NavLink key={to} to={to} end={end} className={navCls} onClick={closeMobile} title={collapsed ? label : undefined}>
      ...
      <div className="sb-footer">
        <NavLink to="/opciones" className={navCls} onClick={closeMobile} title={collapsed ? 'Opciones' : undefined}>
          <Settings size={17} className="sb-icon" />
          <span className="sb-label">Opciones</span>
        </NavLink>
        {session && (
          <button className="sb-link sb-logout" onClick={handleLogout} title={collapsed ? 'Cerrar sesión' : undefined}>
